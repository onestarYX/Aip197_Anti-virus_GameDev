...
namespace GraphMaster {
  public abstract class CreateGraphWizard : EditorWindow
  {
     ...
     static public T CreateGraphGo<T>(GameObject pParent) where T : Graph
     {
     ...
     static public void SetupGraph(Graph pGraph)
     {
     ...
     static public bool ShouldCreate(GameObject go, bool isValid)
     {
        ...
        if(go == null || go.GetComponent<Graph>())
           return false;
        
        if (retVal && isValid)
        {
           Utils.RegisterUndo("Add Graph");
           return true;
        }
        return false;
     }
